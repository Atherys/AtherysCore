...
public final class PartyManager extends AbstractMongoDatabaseManager<Party> {
    ...
    /**
     * Used to get the UUIDs of the members in the party. Is less intensive than {@link
     * PartyManager#getPartyMembers(Party)} since it does not look up the User object of each UUID.
     *
    ...
    /**
     * Used to get the members in the party. Is more intensive than {@link
     * PartyManager#getPartyMemberUUIDs(Party)} since it uses {@link UserUtils} to look up all UUIDs
     * and get their respective users. Is a user could not be found based on their UUID, they are not
     * included in the final list of party members.
     *
    ...
    public List<User> getPartyMembers(Party party) {
        ...
        playerPartyMap.forEach((k, v) -> {
            if (v.getUUID().equals(party.getUUID())) {
                ...
                if (user.isPresent()) {
                    partyMembers.add(user.get());
                } else {
                    playerPartyMap.remove(k);
                }
            }
        });
    ...
    public List<Party> getParties() {
        ...
        playerPartyMap.values().forEach(party -> {
            if (!uniqueParties.contains(party)) {
    ...
    /**
     * Used to set the {@link Party} of a player. WARNING: This does not check if a player is in
     * another party beforehand. This will effectively change the player's party if it is used on a
     * player who is already in another party.
     *
    ...
    /**
     * Adds a new party to the {@link PartyManager}. This will use {@link
     * PartyManager#setPlayerParty(User, Party)} to change every provided player's party.
     *
    ...
    /**
     * Removes a party from the {@link PartyManager}. This will remove every player's UUID whose party
     * is the same as the one provided, therefore making the corresponding players party-less.
     *
